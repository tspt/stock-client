...
export function LaggingSectorTable() {
  const { laggingSectors, loading } = useHotStore();
  ...
  const columns: ColumnsType<HotSector> = [
    {
      ...
      width: 60,
      render: (_, __, index) => (
        <span style={{ fontWeight: 'bold', color: index < 3 ? '#52c41a' : 'inherit' }}>
          {index + 1}
        </span>
      ),
    },

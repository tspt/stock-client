...
function calculateStockCharacteristics(klineData: KLineData[]): StockCharacteristics {
  ...
  if (bb.upper.length > 0 && bb.lower.length > 0) {
    ...
    for (let i = 0; i < bb.upper.length; i++) {
      if (bb.upper[i] !== null && bb.lower[i] !== null && bb.upper[i]! - bb.lower[i]! > 0) {
        const pos = (closes[Math.min(i + 19, closes.length - 1)] - bb.lower[i]!) / (bb.upper[i]! - bb.lower[i]!);
        positions.push(pos);
      }
    }
  ...
  const volVariance = volumes.reduce((acc, val) => acc + Math.pow(val - volMean, 2), 0) / volumes.length;
...
function analyzeBollingerBands(
  ...
  if (position < lowerThreshold) {
    signals.push({ direction: 'up', weight: 0.2, reason: `价格接近布林带下轨（位置${(position * 100).toFixed(1)}%）` });
  } else if (position > upperThreshold) {
    signals.push({ direction: 'down', weight: 0.2, reason: `价格接近布林带上轨（位置${(position * 100).toFixed(1)}%）` });

 ...
 * AI 辅助分析 v7.0
 *
 * 由 v5.0（ai-v5.0.ts）完整拷贝后独立演进：不使用 import ./ai-v5.0。
 * 在 v5.0 相同逻辑基础上，新增安全评分增强：
 * 1. 分层加权安全评分 - 基本面/流动性/波动/结构四维度独立计分后合成
 * 2. 防追高强化 - 当日涨幅、假突破、动能衰竭联动安全分
 * 3. 保留 v5 全部能力 - 假突破过滤、动能衰竭检测、动态权重、v4 结构校正
 *
 * v5.0 源文件保持不变。
 */
 ...
 * 检测市场状态（趋势市 vs 震荡市）
 */
function detectMarketRegime(klineData: KLineData[]): 'trending' | 'ranging' {
  const len = klineData.length;
  if (len < 20) return 'ranging'; // 数据不足默认震荡市
  ...
  // ATR比率 < 0.02 判定为震荡市，否则为趋势市
  return atrRatio < 0.02 ? 'ranging' : 'trending';
}

/**
 * 基于多指标综合判断的趋势预测（v5.0：支持动态权重）
 */
export function predictTrend(
  ...
  if (config.useMovingAverages) {
    ...
    // 应用动态权重
    maSignals.forEach((s) => {
      s.weight *= weights.ma / 0.3; // 归一化到原始基准权重0.3
    });
  ...
  if (config.useRSI) {
    ...
    rsiSignals.forEach((s) => {
      s.weight *= weights.rsi / 0.2; // 归一化到原始基准权重0.2
    });
  ...
  if (config.useMACD) {
    ...
    macdSignals.forEach((s) => {
      s.weight *= weights.macd / 0.25; // 归一化到原始基准权重0.25
    });
  ...
  if (config.useBollingerBands) {
    ...
    bbSignals.forEach((s) => {
      s.weight *= weights.bb / 0.2; // 归一化到原始基准权重0.2
    });
  ...
  trendSignals.forEach((s) => {
    s.weight *= weights.trend / 0.25; // 归一化到原始基准权重0.25
  });
  ...
  volumeSignals.forEach((s) => {
    s.weight *= weights.volume / 0.2; // 归一化到原始基准权重0.2
  });
...
/** v7.0：安全评分子维度 clamp */
function riskClampScore(n: number): number {
  return Math.min(100, Math.max(0, Math.round(n)));
}

/** v7.0：基本面安全（PE、市值） */
function calculateFundamentalSafetyScore(
  data: StockOpportunityData,
  reasons: string[],
  warnings: string[]
): number {
  ...
  if (data.peRatio === undefined) {
    score -= 8;
    warnings.push('缺少PE数据');
  } else if (data.peRatio <= 0) {
    score -= 12;
    warnings.push('亏损或无有效PE');
  } else if (data.peRatio > 100) {
    score -= 20;
    warnings.push(`市盈率过高(${data.peRatio.toFixed(1)})`);
  } else if (data.peRatio > 60) {
    score -= 10;
  } else if (data.peRatio >= 15 && data.peRatio <= 40) {
    score += 12;
    reasons.push('市盈率合理');
  } else if (data.peRatio > 0 && data.peRatio < 15) {
    score += 6;
  }

  const cap = data.circulatingMarketCap ?? data.marketCap;
  if (cap !== undefined) {
    const capYi = cap / 1e8;
    if (capYi < 80) {
      score -= 12;
      warnings.push('流通市值偏小');
    } else if (capYi < 150) {
      score -= 5;
    } else if (capYi >= 150 && capYi <= 800) {
      score += 8;
      reasons.push('市值适中');
    }
  }

  return riskClampScore(score);
}

/** v7.0：流动性安全（换手率） */
function calculateLiquiditySafetyScore(
  data: StockOpportunityData,
  reasons: string[],
  warnings: string[]
): number {
  ...
  if (data.turnoverRate === undefined) {
    score -= 5;
  } else if (data.turnoverRate > 25) {
    score -= 20;
    warnings.push('换手率极高，筹码不稳定');
  } else if (data.turnoverRate > 15) {
    score -= 10;
    warnings.push('换手率偏高');
  } else if (data.turnoverRate >= 2 && data.turnoverRate <= 10) {
    score += 10;
    reasons.push('换手率健康');
  } else if (data.turnoverRate < 1) {
    score -= 8;
    warnings.push('换手过低，流动性不足');
  }

  return riskClampScore(score);
}

/** v7.0：波动安全（历史波动率 + 当日涨跌幅） */
function calculateVolatilitySafetyScore(
  klineData: KLineData[],
  data: StockOpportunityData,
  reasons: string[],
  warnings: string[]
  ...
  if (len >= 20) {
    ...
    if (volatility > 0.08) {
      ...
      score += 10;
      reasons.push('波动率低，走势平稳');
    }
  }

  if (data.changePercent !== undefined) {
    if (data.changePercent > 7) {
      score -= 15;
      warnings.push('当日涨幅过大，追高风险');
    } else if (data.changePercent > 5) {
      score -= 8;
    } else if (data.changePercent >= -3 && data.changePercent <= 3) {
      score += 5;
      reasons.push('当日涨跌温和');
    } else if (data.changePercent < -7) {
      score -= 6;
      warnings.push('当日跌幅较大');
    }
  }
...
/** v7.0：结构安全（均线、横盘、异动、回撤） */
function calculateStructureSafetyScore(
  klineData: KLineData[],
  data: StockOpportunityData,
  reasons: string[],
  warnings: string[]
  ...
  if (data.ma5 !== undefined && data.ma10 !== undefined && data.ma20 !== undefined && price !== undefined) {
    if (price > data.ma5 && data.ma5 > data.ma10 && data.ma10 > data.ma20) {
      score += 15;
      reasons.push('均线多头排列，结构稳健');
    } else if (price < data.ma20) {
      score -= 12;
      warnings.push('价格在MA20下方');
    } else if (price < data.ma5 && data.ma5 < data.ma10) {
      score -= 10;
      warnings.push('短期均线空头');
    }
  }

  if (data.ma20 !== undefined && data.ma60 !== undefined && data.ma20 > data.ma60) {
    score += 5;
  }
  ...
  if (data.sharpMovePatterns?.onlyDrop) {
    score -= 15;
    warnings.push('近期急跌');
  }
  if (data.sharpMovePatterns?.riseThenDropLoose || data.sharpMovePatterns?.riseThenFlatThenDrop) {
    score -= 10;
    warnings.push('急涨后回落');
  }
  if (data.sharpMovePatterns?.dropThenFlatThenRise) {
    score += 8;
    reasons.push('急跌后企稳反弹');
  }
  ...
  if (len >= 20) {
    ...
    if (high20 > 0) {
      ...
      if (pullback > 0.15) {
        score -= 12;
        warnings.push('距20日高点回撤较大');
      } else if (pullback < 0.05 && (data.changePercent === undefined || data.changePercent <= 5)) {
        score += 6;
        reasons.push('接近20日高点，趋势较强');
      }
    }
  }
...
function calculateRiskScore(
  ...
  const fundamentalScore = calculateFundamentalSafetyScore(data, reasons, warnings);
  const liquidityScore = calculateLiquiditySafetyScore(data, reasons, warnings);
  const volatilityScore = calculateVolatilitySafetyScore(klineData, data, reasons, warnings);
  const structureScore = calculateStructureSafetyScore(klineData, data, reasons, warnings);
...
function v4ApplyStructuralAdjust(klineData: KLineData[], base: AIAnalysisResult): AIAnalysisResult {
  ...
  if (rd) {
    const weakShortMA =
      rd.ma5 !== undefined && rd.ma10 !== undefined && rd.lastClose < rd.ma5 && rd.ma5 < rd.ma10;
    const bullishShortMA =
      rd.ma5 !== undefined && rd.ma10 !== undefined && rd.lastClose > rd.ma5 && rd.ma5 > rd.ma10;

    const pull = rd.high20 !== undefined ? v4PullbackFromHigh(rd.high20, rd.lastClose) : 0;
    ...
    if (tp.direction === 'up') {
      if (rd.trailingDownCloses >= 3) {
        ...
        dRisk -= 6;
        confMul *= 0.82;
        rec.warnings.push('v7：近端连续多根收盘价走低，与看涨结论冲突，已下调评分与置信度');
      } else if (rd.trailingDownCloses >= 2) {
        dTrend -= 8;
        dRisk -= 3;
        confMul *= 0.88;
        rec.warnings.push('v7：近端连阴，弱势确认中，已适度下调评分');
      }

      if (weakShortMA) {
        dTrend -= 10;
        confMul *= 0.9;
        rec.warnings.push('v7：价在短期均线之下且均线空头，追高风险较高');
      }

      if (rd.ret5 !== undefined && rd.ret5 < -4) {
        dTrend -= 6;
        confMul *= 0.92;
        rec.warnings.push('v7：近5根K线回报偏弱，注意下跌惯性');
      }

      if (pull > 0.14) {
        dTrend -= 5;
        confMul *= 0.94;
        rec.warnings.push('v7：距20日高点回撤较大，趋势看涨与位置需复核');
      }

      if (bullishShortMA && rd.trailingDownCloses === 0 && pull < 0.08) {
        dTrend += 4;
        dRisk += 2;
        rec.reasons.unshift('v7：短周期均线多头且未见连阴，与趋势判断一致');
      }
    }
    ...
    if (tp.direction === 'sideways') {
      if (bullishShortMA && rd.ret5 !== undefined && rd.ret5 > 0.8) {
        dTrend += 5;
        rec.reasons.unshift('v7：横盘背景下短均线多头且近端偏强，可适当关注');
      }
    }

    if (tp.direction === 'down' && rd.trailingDownCloses >= 3 && weakShortMA) {
      dRisk -= 5;
      rec.warnings.push('v7：下跌方向下短周期仍走弱，注意承接风险');
    }
  }
  ...
  if (rd && confMul < 0.999) {
    tp.reasoning.unshift(`v7：短周期结构校正，置信度系数×${confMul.toFixed(2)}`);
...
// ========== v7.0：增强版结构调整（整合 v4.0 + 假突破 + 动能衰竭 + 安全分增强） ==========

function v7ApplyEnhancedAdjust(klineData: KLineData[], base: AIAnalysisResult): AIAnalysisResult {
  // 先应用 v4.0 的短周期结构校正
  ...
  if (falseBreakout.isFalseBreakout) {
    ...
    if (falseBreakout.breakoutType === 'up') {
      // 向上假突破：无论趋势方向，均降低安全分（防追高）
      dRisk -= severityPenalty * 0.4;
    }
    ...
    if (falseBreakout.breakoutType === 'up' && tp.direction === 'up') {
      ...
      rec.warnings.push(
        `v7：检测到向上假突破（严重程度${(falseBreakout.severity * 100).toFixed(
          0
        )}%），已大幅下调评分`
      );
      ...
      confMul *= 1 - falseBreakout.severity * 0.3;
      rec.warnings.push(
        `v7：检测到向下假突破（严重程度${(falseBreakout.severity * 100).toFixed(
          0
        )}%），已大幅下调评分`
      );
    }
  ...
  if (momentum.exhausted) {
    ...
    if (momentum.direction === 'up' && tp.direction === 'up') {
      ...
      dRisk -= exhaustionPenalty * 0.35;
      confMul *= 1 - momentum.exhaustionLevel * 0.2;
      rec.warnings.push(
        `v7：上涨动能衰竭（程度${(momentum.exhaustionLevel * 100).toFixed(0)}%），警惕回调`
      );
      ...
      confMul *= 1 - momentum.exhaustionLevel * 0.2;
      rec.warnings.push(
        `v7：下跌动能衰竭（程度${(momentum.exhaustionLevel * 100).toFixed(0)}%），可能反弹`
      );
    }
  }
  ...
  if (confMul < 0.999) {
    tp.reasoning.unshift(`v7：增强校正，置信度系数×${confMul.toFixed(2)}`);
    tp.reasoning = tp.reasoning.slice(0, 5);
  }
...
export function performAIAnalysis(
  ...
  // 应用 v7.0 增强校正（包含 v4.0 短周期结构 + 假突破 + 动能衰竭 + 安全分增强）
  return v7ApplyEnhancedAdjust(klineData, {
    trendPrediction,
    similarPatterns,
    recommendation,
    analyzedAt: Date.now(),

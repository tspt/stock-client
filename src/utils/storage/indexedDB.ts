 ...
 * 保存分析结果
 */
export async function saveOverviewData(data: OverviewAnalysisResult): Promise<void> {
 ...
 * 保存历史快照
 */
export async function saveOverviewHistory(data: OverviewAnalysisResult): Promise<void> {
  const db = await initDB();
 ...
 * 获取历史记录列表
 */
export async function getOverviewHistory(limit: number = 10): Promise<OverviewAnalysisResult[]> {
  const db = await initDB();

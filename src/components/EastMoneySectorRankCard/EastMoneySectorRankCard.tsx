  ...
  return (
    ...
    >
      ...
      <div className={styles.listHeader}>
        ...
        <span className={styles.headerCell} style={{ flex: 1 }}>领涨股</span>
      </div>
        ...
        {data.map((item, index) => {
          ...
          return (
            <div key={item.code} className={styles.listItem}>
              ...
              <Text className={styles.sectorName} ellipsis={{ tooltip: item.name }} style={{ width: 140 }}>
                {item.name}
              </Text>
              <span className={`${changeColor} ${styles.changePercentBold}`} style={{ width: 80, textAlign: 'right' }}>
                {isPositive ? '+' : ''}{item.changePercent.toFixed(2)}%
              </span>
              <span className={styles.riseCount} style={{ width: 60, textAlign: 'center' }}>{item.riseCount ?? '-'}</span>
              <span className={styles.fallCount} style={{ width: 60, textAlign: 'center' }}>{item.fallCount ?? '-'}</span>
              <div className={styles.leadingStock} style={{ flex: 1 }}>
                ...
                <Text className={leadingStockColor} style={{ fontSize: 12, flexShrink: 0 }}>
                  {leadingStockPositive ? '+' : ''}{item.leadingStockChangePercent.toFixed(2)}%
                </Text>
              </div>
            </div>
          );

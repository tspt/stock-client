...
import styles from './ConceptSectorStocksDrawer.module.css';

const { Text } = Typography;

interface ConceptSectorStocksDrawerProps {
  open: boolean;
  sectorCode: string;
  sectorName: string;
  onClose: () => void;
...
export function ConceptSectorStocksDrawer({
  open,
  sectorCode,
  sectorName,
  onClose,
  ...
  const loadData = useCallback(async (page: number = 1) => {
    if (!sectorCode || isLoadingRef.current) return;
    ...
    try {
      ...
      logger.error('加载概念板块股票数据失败:', error);

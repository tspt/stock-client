 ...
 */
export function initProxySession() {
    if (!proxySession) {
            ...
            const sendUpstream = (attempt) => {
                ...
                if (useDirectHttps && uReq.protocol === 'https:') {
                    ...
                    if (attempt > 0) {
                        ...
                        console.warn(`[embedded-proxy] 东财 切换上游主机: ${uReq.hostname} (第${attempt}次重试，Node https)`);
                    }
                    // 调试日志：输出请求信息
                    const poolCookie = getEastmoneyPoolCookie(req);
                    ...
                    const proxyReq = https.request(requestOpt, (proxyRes) => {
                        if (prefix === '/api/eastmoney') {
                            if (attempt > 0) {
                                ...
                                console.log('[代理调试] 东方财富响应 (Node https):', proxyRes.statusCode, proxyRes.statusMessage);
                            }
                        }
                        // 检查是否需要解压缩

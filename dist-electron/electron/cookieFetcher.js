...
// 简单的日志函数，避免依赖 src 目录
const log = (msg, ...args) => console.log(`[CookieFetcher] ${msg}`, ...args);
const warn = (msg, ...args) => console.warn(`[CookieFetcher] ${msg}`, ...args);
const error = (msg, ...args) => console.error(`[CookieFetcher] ${msg}`, ...args);
// 取消标志
let isCancelled = false;
/**
 * 发送进度更新到渲染进程
 */
function sendProgress(window, progress) {
    if (window) {

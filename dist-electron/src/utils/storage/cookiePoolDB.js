 ...
 */
import { COOKIE_POOL_DB_NAME, COOKIE_POOL_DB_VERSION, COOKIE_POOL_STORE_NAME, } from '../config/constants.js';
import { logger } from '../business/logger.js';
let dbInstance = null;
/**
 * 初始化Cookie池数据库
 */
export async function initCookiePoolDB() {
    if (dbInstance) {
        return dbInstance;
    }
 ...
 * 更新Cookie健康状态
 */
export async function updateCookieHealth(id, success) {
    const db = await initCookiePoolDB();
    const transaction = db.transaction([COOKIE_POOL_STORE_NAME], 'readwrite');
    const store = transaction.objectStore(COOKIE_POOL_STORE_NAME);
    return new Promise((resolve, reject) => {
        ...
        getRequest.onsuccess = () => {
            const cookie = getRequest.result;
            if (!cookie) {
 ...
 */
export async function exportCookies(filter = 'all') {
    const db = await initCookiePoolDB();
    const transaction = db.transaction([COOKIE_POOL_STORE_NAME], 'readonly');
    const store = transaction.objectStore(COOKIE_POOL_STORE_NAME);
    return new Promise((resolve, reject) => {
 ...
 */
export async function importCookies(cookies, skipDuplicates = true) {
    const db = await initCookiePoolDB();
    let successCount = 0;
    let skippedCount = 0;
    const duplicateIds = [];

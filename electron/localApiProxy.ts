    ...
    const server = http.createServer((req, res) => {
      ...
      const sendUpstream = (attempt: number): void => {
        ...
        if (prefix === '/api/eastmoney' && uReq.protocol === 'https:') {
          ...
          if (attempt > 0) {
            console.warn(
              `[embedded-proxy] 东财 切换上游主机: ${uReq.hostname} (第${attempt}次重试，Chromium net.request)`
            );
          }
          ...
          creq.on('response', (proxyRes) => {
            ...
            (proxyRes as unknown as import('http').IncomingMessage).pipe(res);
          });
          creq.on('error', (err: NodeJS.ErrnoException) => {
          ...
          if (req.method === 'GET' || req.method === 'HEAD') {
            ...
            req.pipe(creq as unknown as import('net').Socket);
          }
          return;
        }

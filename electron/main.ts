...
function setupIpcHandlers() {
  ...
  ipcMain.handle('test-cookie', async (event, cookieValue: string) => {
    try {
    ...
    async (
      ...
      try {
        ...
        if (!existsSync(stockDataDir)) {
          ...
          mainLog(`[主进程] 清空股票数据目录: ${stockDataDir}`);
  ...
  function getLatestBuyPointDir(): string {
    if (isDev) {
   ...
   */
  ipcMain.handle(
    'read-latest-buy-point-files',
    async (_event, payload?: { dates?: string[] } | null) => {
      try {
        const targetDir = getLatestBuyPointDir();
        if (!existsSync(targetDir)) {
        ...
        const targets = requested
          ? allFiles.filter((fileName) =>
              requested.has(fileName.replace(/\.json$/i, ''))
            )
          : allFiles;
  ...
  function getOpportunityRecordDir(): string {
    if (isDev) {
  ...
  function ensureOpportunityRecordDir(): string {
   ...
   */
  ipcMain.handle(
    'write-opportunity-record-file',
    async (_event, payload: { fileBaseName: string; content: string }) => {
      try {
   ...
   * 删除指定日期的机会记录文件
   */
  ipcMain.handle('delete-opportunity-record-file', async (_event, date: string) => {
    try {
  ...
  function ensureHotRankDir(): string {
  ...
  /** 热门榜文件名：YYYY-MM-DD（24小时）或 YYYY-MM-DD_HH（1小时） */
  const HOT_RANK_FILE_BASE_PATTERN = /^[0-9]{4}-[0-9]{2}-[0-9]{2}(_[0-9]{2})?$/;
  ...
  ipcMain.handle(
    'write-hot-rank-file',
    async (_event, payload: { fileBaseName: string; content: string; period?: 'hour' | 'day' }) => {
      try {
        const { fileBaseName, content, period } = payload;
        if (!fileBaseName || !HOT_RANK_FILE_BASE_PATTERN.test(fileBaseName)) {
          return { success: false, error: '非法文件名，需为 YYYY-MM-DD 或 YYYY-MM-DD_HH 格式' };
        }
        ...
        if (period === 'hour' || period === 'day') {
          ...
          const now = Date.now();
          const calendarDate = fileBaseName.slice(0, 10);
          ...
          toWrite = JSON.stringify(envelope, null, 2);
        }
   ...
   * 不存在时 success=true 且 content=null（不读带小时后缀的文件）
   */
  ipcMain.handle('read-hot-rank-day-file', async (_event, date: string) => {
    try {
      if (!date || !OPPORTUNITY_RECORD_DATE_PATTERN.test(date)) {
        return { success: false, error: '非法日期，需为 YYYY-MM-DD 格式' };
      }
      ...
      const filePath = join(targetDir, `${date}.json`);
      if (!existsSync(filePath)) {
        return { success: true, exists: false, content: null, filePath };
      }

      const content = JSON.parse(readFileSync(filePath, 'utf-8'));
      return { success: true, exists: true, content, filePath };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      mainLog(`[主进程] 读取热门榜失败: ${errorMessage}`, true);
      return { success: false, error: errorMessage };
    }
  });

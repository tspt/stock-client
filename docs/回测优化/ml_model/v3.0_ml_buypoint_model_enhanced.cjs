...
console.log('================================================================================');
console.log('=== 基于决策树的买点识别模型 - 增强版参数优化 ===');
console.log('================================================================================\n');
...
async function runOptimization() {
  ...
  for (const config of parameterGrid) {
    ...
    // 步骤1: 构建数据集
    console.log('\n步骤1: 构建训练数据集');
    const { X, y, positiveSamples, missedBuypoints } = buildDataset(config.negativeSamplesPerStock);
    ...
    // 步骤4: 分析未捕获买点
    console.log(`\n  未捕获买点: ${missedBuypoints.length}个`);
    if (missedBuypoints.length > 0) {
      const avgFeatures = calculateAverageFeatures(missedBuypoints);
      console.log(`  平均特征:`);
...
function buildDataset(negativeSamplesPerStock) {
  ...
  // 找出未捕获的买点（用于后续分析）
  const missedBuypoints = positiveSamples;

  return { X, y, positiveSamples, missedBuypoints };
}
